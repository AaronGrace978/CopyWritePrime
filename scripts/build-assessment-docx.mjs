...
const p = (text, extra = {}) =>
  new Paragraph({ spacing: DOUBLE, ...extra, children: [run(text, extra.run)] });

const h = (text, level = HeadingLevel.HEADING_1) =>
  new Paragraph({
    heading: level,
    spacing: DOUBLE,
    children: [run(text, { bold: level !== HeadingLevel.TITLE, italics: level === HeadingLevel.HEADING_3 })],
  });
